...
|---------------------------------------|---------------------------------|
|`lucid()`| The main function to fit LUCID model. It combines functionality of `est_lucid()` and `tune_lucid()` |
| `est_lucid()` | The function for estimation. It takes multi-view data as input and returns a LUCID model.|
| `tune_lucid()` | The function for model selection. It fits LUCID models across the grid of tuning parameters, including number of latent clusters $K$ and regularization penalties. It returns the optimal model based on BIC |
| `boot_lucid()` | The function for inference. It returns confidence intervals (CI) with given confidence level by using bootstrap resampling. |
| `summary_lucid()` | The function for summary. It returns a table of coefficients in the LUCID model. |
| `plot_lucid()` | The function for visualization. It creates a Sankey diagram to visualize LUCID model by using user-defined palette. |
